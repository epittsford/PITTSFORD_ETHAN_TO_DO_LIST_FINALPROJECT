...
def load_existing_list():
    try:
        with open("todolist.txt", "r") as f:
            app.tasks = [line.strip() for line in f.readlines()]
        app.refresh_listbox()
    except FileNotFoundError:
        tk.messagebox.showerror("Error", "No saved list found.")
    ...
    file_menu.add_command(label="Create New List", command=create_new_list)
    file_menu.add_command(label="Load Existing List", command=load_existing_list)
    ...
    self.load_button = tk.Button(self.mid_frame, text="Load List", command=load_existing_list)
    self.load_button.pack(side="left")
